        ...
        try:
            ...
            try:
                ...
                if not parsed.scheme:
                    self.errors.append("DATABASE_URL missing scheme (e.g., sqlite+aiosqlite)")
                elif parsed.scheme not in ["sqlite+aiosqlite", "postgresql+asyncpg", "mysql+aiomysql"]:
                    self.warnings.append(f"DATABASE_URL uses uncommon scheme: {parsed.scheme}")
                else:
                ...
                if "sqlite" in parsed.scheme:
                    if not parsed.path or parsed.path == "/":
                        self.errors.append("SQLite DATABASE_URL missing database file path")
                    else:
                ...
                # PostgreSQL/MySQL checks
                    ...
                    if not parsed.password:
                        self.warnings.append("Database URL missing password (may be in environment)")
    ...
    def _validate_server_settings(self) -> None:
        ...
        try:
            ...
            if not isinstance(allowed_origins, list):
                ...
                if settings.get("ENV") == "production":
                    self.errors.append("ALLOWED_ORIGINS cannot contain '*' in production")
                else:
                    self.warnings.append("ALLOWED_ORIGINS contains '*' (development only)")
            else:
                self.info.append(f"CORS origins configured: {len(allowed_origins)} domains")
    ...
    def _validate_email_settings(self) -> None:
        ...
        try:
            ...
            if not smtp_host and not smtp_username:
                self.info.append("Email not configured (password reset will use console output)")
                return
    ...
    def _validate_application_settings(self) -> None:
        ...
        try:
            ...
            if not qr_base_url.startswith(("http://", "https://")):
                self.errors.append("QR_CODE_BASE_URL must start with http:// or https://")
            else:
                self.info.append(f"QR code base URL: {qr_base_url}")
            
            # PDF settings
            badges_per_page = settings.get("PDF_BADGES_PER_PAGE", 35)
            if not isinstance(badges_per_page, int) or not (1 <= badges_per_page <= 100):
                self.errors.append(f"PDF_BADGES_PER_PAGE must be between 1 and 100, got: {badges_per_page}")
            else:
    ...
    def _test_smtp_connection(self) -> None:
        ...
        try:
            ...
            if result == 0:
                ...
                self.warnings.append(f"Cannot connect to SMTP server {smtp_host}:{smtp_port}")
