...
class ConnectionManager:
    ...
    async def broadcast_to_organizers(
        self, message: dict, event_id: uuid.UUID | None = None
    ):
    ...
    def get_connection_stats(self) -> dict:
        ...
        return {
            ...
            "organizer_connections": len(
                [c for c in self.connection_metadata.values() if c.get("is_organizer")]
            ),
        }
...
class RoundTimerManager:
    ...
    async def start_round_timer(
        ...
        print(
            f"Started timer for round {round_id} ({duration_minutes}m + {break_minutes}m break)"
        )
    ...
    async def _run_round_timer(
        self, round_id: uuid.UUID, session: AsyncSession | None = None
    ):
        ...
        try:
            ...
            # Round phase
            for _remaining in range(duration_seconds, -1, -1):
                ...
                await self.connection_manager.broadcast_to_round_timer(
                    {
                        ...
                        "status": "active",
                        "percentage_complete": (elapsed / duration_seconds) * 100
            ...
            if break_seconds > 0:
                ...
                for _remaining in range(break_seconds, -1, -1):
                    if round_id not in self.timer_data:  # Timer was stopped
                        return
                    ...
                    await self.connection_manager.broadcast_to_round_timer(
                        {
                            ...
                            "status": "break",
                            "percentage_complete": (break_elapsed / break_seconds) * 100
                            if break_seconds > 0
                            else 100,
                        },
                        round_id,
    ...
    def get_timer_status(self, round_id: uuid.UUID) -> dict | None:
    ...
    def get_all_active_timers(self) -> list[dict]:
        ...
        return [
            self.get_timer_status(round_id) for round_id in self.active_timers.keys()
        ]
...
class EventCountdownManager:
    ...
    async def extend_event_countdown(
        self, event_id: uuid.UUID, additional_minutes: int
    ):
        ...
        new_target = countdown_data["target_time"] + timedelta(
    ...
    async def _run_event_countdown(
        ...
        try:
            ...
            while True:
                ...
                percentage = (
                    (elapsed_seconds / total_seconds * 100) if total_seconds > 0 else 0
                )
                ...
                if (
                    ...
                    and "10min" not in warnings_sent
                    and minutes_remaining >= 9
                ):  # 10 minutes
                    ...
                    remaining_seconds <= 60 and "1min" not in warnings_sent
                ):  # 1 minute
                    ...
                    remaining_seconds <= 30 and "30sec" not in warnings_sent
                ):  # 30 seconds
                    ...
                    remaining_seconds <= 10 and "10sec" not in warnings_sent
                ):  # 10 seconds
    ...
    def get_countdown_status(self, event_id: uuid.UUID) -> dict | None:
