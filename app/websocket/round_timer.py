...
from app.models import User, Event, Round, RoundStatus
...
class RoundTimerHandler:
        ...
        try:
            ...
            while True:
                try:
                    ...
                    await self.handle_message(message, connection_id, event_id, user_id, session)
    ...
    async def handle_message(
        ...
        if message_type == "heartbeat":
            ...
            if round_id:
                await self.subscribe_to_round(uuid.UUID(round_id), connection_id, session)
        
        elif message_type == "request_timer_update":
            round_id = message.get("round_id")
            if round_id:
                await self.send_round_timer_update(uuid.UUID(round_id), connection_id, session)
    ...
    async def calculate_timer_state(self, round_obj: Round) -> Dict[str, Any]:
        ...
        timer_state = {
            ...
            "scheduled_start": round_obj.scheduled_start.isoformat() if round_obj.scheduled_start else None,
    ...
    async def _round_timer_task(
        ...
        try:
            ...
            while updates_sent * update_interval < duration_seconds:
                ...
                if remaining in [120, 60, 30, 10]:  # 2 min, 1 min, 30 sec, 10 sec warnings
                    await connection_manager.broadcast_to_event({
                        "type": "timer_warning",
                        "round_id": str(round_id),
            ...
            # Final timer update
            await self.broadcast_round_update(round_id, event_id, session, "round_completed")
