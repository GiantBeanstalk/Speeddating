...
from app.models import (
    User, Event, EventStatus, Attendee, AttendeeCategory, Match, MatchResponse
)
...
class AttendeeUpdate(BaseModel, ContactValidationMixin):
    ...
    contact_phone: Optional[str] = Field(None, max_length=25)  # Increased for formatted numbers
    fetlife_username: Optional[str] = Field(None, max_length=100)
    contact_visible_to_matches: Optional[bool] = None
...
async def get_my_registrations(
    ...
    return [
        build_attendee_response(attendee, current_user)
...
async def get_event_attendees(
    event_id: uuid.UUID,
    category: Optional[AttendeeCategory] = Query(None),
    ...
    if checked_in_only:
        query = query.where(Attendee.checked_in == True)
    
    if confirmed_only:
        query = query.where(Attendee.registration_confirmed == True)
...
async def confirm_payment(
    ...
    status_message = "confirmed" if payment_data.payment_confirmed else "marked as unpaid"
...
async def get_attendee_matches(
    ...
    for match in matches:
        ...
        other_attendee = match.attendee2 if match.attendee1_id == attendee_id else match.attendee1
        my_response = match.attendee1_response if match.attendee1_id == attendee_id else match.attendee2_response
        their_response = match.attendee2_response if match.attendee1_id == attendee_id else match.attendee1_response
    ...
    return {
        ...
        "completed_responses": sum(1 for m in match_data if m["my_response"] != "NO_RESPONSE"),
