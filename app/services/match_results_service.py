...
from app.models import Attendee, Event, Match, MatchResponse
...
class MatchResultsService:
    ...
    async def get_event_match_statistics(self, event_id: uuid.UUID) -> dict[str, Any]:
        ...
        total_possible_responses = (
            total_matches * 2
        )  # Each match has 2 potential responses
        ...
        return {
            ...
            "response_rate": (actual_responses / total_possible_responses) * 100
            if total_possible_responses > 0
            else 0,
            "mutual_match_rate": (len(mutual_matches) / total_matches) * 100,
            "success_rate": (len(mutual_matches) / len(responded_matches)) * 100
            if responded_matches
            else 0,
    ...
    async def get_mutual_matches_for_event(
        ...
        return [
            {
                ...
                "attendee1": {
                    ...
                    "response_time": match.attendee1_response_time.isoformat()
                    if match.attendee1_response_time
                    else None,
                ...
                "attendee2": {
                    ...
                    "response_time": match.attendee2_response_time.isoformat()
                    if match.attendee2_response_time
                    else None,
        ...
        for match in matches:
            writer.writerow(
                [
                    ...
                    match.attendee1_response_time.isoformat()
                    if match.attendee1_response_time
                    else "",
                    ...
                    match.attendee2_response_time.isoformat()
                    if match.attendee2_response_time
                    else "",
    ...
    async def export_mutual_matches_csv(self, event_id: uuid.UUID) -> io.StringIO:
        ...
        writer.writerow(
            [
                ...
                "Person 1 Category",
                "Person 1 Age",
                "Person 1 Rating Given",
                "Person 2 Name",
                "Person 2 Category",
                "Person 2 Age",
                "Person 2 Rating Given",
                "Match Created At",
            ]
        )
        ...
        for attendee in attendees:
            ...
            writer.writerow(
                [
                    ...
                    round(stats["avg_rating_given"], 1)
                    if stats["avg_rating_given"]
                    else "",
                    round(stats["avg_rating_received"], 1)
                    if stats["avg_rating_received"]
                    else "",
                ]
            )
    ...
    async def get_attendee_detailed_results(
        ...
        for match in matches:
            ...
            my_response = (
                match.attendee1_response if is_attendee1 else match.attendee2_response
            )
            other_response = (
                match.attendee2_response if is_attendee1 else match.attendee1_response
            )
            my_rating = (
                match.attendee1_rating if is_attendee1 else match.attendee2_rating
            )
            other_rating = (
                match.attendee2_rating if is_attendee1 else match.attendee1_rating
            )
    ...
    async def _calculate_attendee_response_rates(
        ...
        for attendee_id, stats in attendee_stats.items():
            response_rate = (
                (stats["responses_given"] / stats["total_matches"]) * 100
                if stats["total_matches"] > 0
                else 0
            )
            yes_rate = (
                (stats["yes_responses"] / stats["responses_given"]) * 100
                if stats["responses_given"] > 0
                else 0
            )
    ...
    async def _calculate_round_statistics(
        self, matches: list[Match]
    ) -> list[dict[str, Any]]:
        ...
        return sorted(
            result, key=lambda x: (x["round_number"] == "Unknown", x["round_number"])
        )

    async def _calculate_rating_statistics(
        self, matches: list[Match]
    ) -> dict[str, Any]:
        ...
        if not ratings:
            return {"total_ratings": 0, "average_rating": 0, "rating_distribution": {}}
    ...
    async def _calculate_timing_statistics(
        self, matches: list[Match]
    ) -> dict[str, Any]:
    ...
    async def _get_attendee_match_stats(self, attendee_id: uuid.UUID) -> dict[str, Any]:
        ...
        response_rate = (
            (responses_given / total_matches) * 100 if total_matches > 0 else 0
        )
        success_rate = (
            (mutual_matches / responses_given) * 100 if responses_given > 0 else 0
        )
        avg_rating_given = (
            sum(ratings_given) / len(ratings_given) if ratings_given else None
        )
        avg_rating_received = (
            sum(ratings_received) / len(ratings_received) if ratings_received else None
        )
