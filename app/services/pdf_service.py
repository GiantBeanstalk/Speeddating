...
class PDFBadgeService:
    ...
    async def generate_event_badges(
        ...
        for i, attendee in enumerate(attendees):
            ...
            self._draw_single_badge(
                c, attendee, event, x, y, 
                qr_data.get(attendee.id),
                name_style, info_style
            )
        ...
        c.setSubject(f"Event badges generated on {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    def _draw_single_badge(
        ...
        name_frame = Frame(
            content_x, content_y + content_height - 15 * mm - qr_y_offset,
            content_width - (self.qr_size + 2 * mm if qr_login else 0), 10 * mm,
        ...
        category_frame = Frame(
            content_x, content_y + content_height - 25 * mm - qr_y_offset,
            content_width - (self.qr_size + 2 * mm if qr_login else 0), 8 * mm,
            leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0
        )
        ...
        event_frame = Frame(
            content_x, content_y + content_height - 35 * mm - qr_y_offset,
            content_width - (self.qr_size + 2 * mm if qr_login else 0), 8 * mm,
            leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0
        )
    ...
    async def generate_single_badge(
        ...
        # Draw the badge
        self._draw_single_badge(
            c, attendee, attendee.event, 
            badge_x, badge_y, qr_login,
            name_style, info_style
        )
    ...
    async def get_badge_generation_stats(self, event_id: uuid.UUID) -> Dict[str, Any]:
        ...
        pages_needed = (len(confirmed_attendees) + self.badges_per_page - 1) // self.badges_per_page
