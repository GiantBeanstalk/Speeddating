...
class InputSanitizer:
    ...
    SQL_KEYWORDS = {
        ...
        'UNION', 'FROM', 'WHERE', 'JOIN', 'HAVING', 'GROUP', 'ORDER',
    ...
    SQL_OPERATORS = {
        '--', '/*', '*/', ';', '||', '&&', 'xp_', 'sp_', 'fn_',
        '@@', '0x', 'char(', 'nchar(', 'varchar(', 'nvarchar(',
    ...
    def sanitize_text(self, text: str) -> str:
        ...
        if self.config.min_length and len(text) < self.config.min_length:
            raise ValueError(f"Input too short (min {self.config.min_length} characters)")
...
def sanitize_text(text: str, config: Optional[SanitizationConfig] = None) -> str:
