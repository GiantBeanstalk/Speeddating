...
from sqlalchemy import String, Text, Integer, Boolean, DateTime, ForeignKey, func, Table, Column
...
attendee_preferences = Table(
    ...
    Base.metadata,
    Column("attendee_id", ForeignKey("attendee.id", ondelete="CASCADE"), primary_key=True),
...
class Attendee(Base):
    ...
    public_bio: Mapped[Optional[str]] = mapped_column(Text)  # Filtered bio visible to all attendees
    private_bio: Mapped[Optional[str]] = mapped_column(Text)  # Internal notes, only visible to organizers
    ...
    profile_qr_token: Mapped[Optional[str]] = mapped_column(String(100), unique=True)  # For profile viewing
    qr_token: Mapped[Optional[str]] = mapped_column(String(100), unique=True)  # For event login
    qr_generated_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    qr_last_used: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    
    # Contact information (one required for match contact purposes)
    contact_email: Mapped[Optional[str]] = mapped_column(String(320))
    contact_phone: Mapped[Optional[str]] = mapped_column(String(25))  # Increased for formatted UK numbers
    fetlife_username: Mapped[Optional[str]] = mapped_column(String(100))
    
    # Contact visibility flags
    contact_visible_to_matches: Mapped[bool] = mapped_column(Boolean, default=True)
    profile_visible: Mapped[bool] = mapped_column(Boolean, default=True)  # Controls public profile visibility
    
    # Registration status
    registration_confirmed: Mapped[bool] = mapped_column(Boolean, default=False)
    payment_confirmed: Mapped[bool] = mapped_column(Boolean, default=True)  # Default true for free events
    
    # Special requirements or notes
    dietary_requirements: Mapped[Optional[str]] = mapped_column(String(500))
    ...
    def validate_contact_fields(self) -> None:
        ...
        from app.utils.validators import validate_email, validate_uk_phone_number, validate_fetlife_username
        ...
        if self.fetlife_username and not validate_fetlife_username(self.fetlife_username):
    ...
    def get_contact_info(self, requester_is_admin: bool = False, is_matched: bool = False) -> dict:
        """
        ...
        if requester_is_admin:
            ...
            contact_info = {
                "email": None,
                "phone": None,
                "fetlife_username": None
            }
        
        # Remove None values
        return {k: v for k, v in contact_info.items() if v is not None}
    
    def get_public_profile_data(self, requester_is_admin: bool = False, is_matched: bool = False) -> dict:
        """
    ...
    def is_interested_in_category(self, category: AttendeeCategory) -> bool:
        ...
        if self.category == AttendeeCategory.TOP_MALE:
            return category in [AttendeeCategory.TOP_FEMALE, AttendeeCategory.BOTTOM_FEMALE]
    ...
    def can_match_with(self, other: "Attendee") -> bool:
        ...
        return (
            self.is_interested_in_category(other.category) and
            other.is_interested_in_category(self.category)
        )
