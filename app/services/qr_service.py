...
from app.models import Attendee, QRLogin
...
class QRCodeService:
    ...
    async def generate_attendee_qr_token(
        self, attendee_id: uuid.UUID, expire_hours: int = 24, max_uses: int = 10
    ) -> QRLogin:
        ...
        attendee_result = await self.session.execute(
            select(Attendee)
            .options(selectinload(Attendee.user), selectinload(Attendee.event))
    ...
    async def _get_active_qr_token(self, attendee_id: uuid.UUID) -> QRLogin | None:
    ...
    def create_qr_code_image(
        self, qr_url: str, size: int = 10, border: int = 4
    ) -> BytesIO:
    ...
    async def validate_qr_token(
        ...
        qr_result = await self.session.execute(
            ...
            .where(QRLogin.token_hash == token_hash, QRLogin.event_id == event_id)
        )
    ...
    async def revoke_attendee_qr_token(
        self, attendee_id: uuid.UUID, reason: str = "Manually revoked"
    ) -> bool:
    ...
    async def extend_qr_token_expiry(
        self, attendee_id: uuid.UUID, additional_hours: int = 24
    ) -> QRLogin | None:
    ...
    async def get_qr_token_stats(self, event_id: uuid.UUID) -> dict[str, Any]:
        ...
        qr_logins_result = await self.session.execute(
            select(QRLogin).where(QRLogin.event_id == event_id)
        )
        ...
        expired_tokens = sum(
            1 for qr in qr_logins if qr.expires_at < now and not qr.is_revoked
        )
    ...
    async def cleanup_expired_tokens(self, event_id: uuid.UUID | None = None) -> int:
    ...
    async def bulk_generate_qr_tokens(
        self, event_id: uuid.UUID, expire_hours: int = 24, max_uses: int = 10
    ) -> list[QRLogin]:
        ...
        for attendee in attendees:
            try:
                qr_token = await self.generate_attendee_qr_token(
                    attendee.id, expire_hours=expire_hours, max_uses=max_uses
                )
