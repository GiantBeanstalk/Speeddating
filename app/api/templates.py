...
async def login_page(request: Request):
    ...
    return templates.TemplateResponse(
        "auth/login.html", {"request": request, "current_year": datetime.now().year}
    )
...
async def attendee_dashboard(
    ...
    try:
        ...
        # Get recent matches
        recent_matches = await get_user_recent_matches(
            session, current_user.id, limit=5
        )
...
async def home_page(
    ...
    try:
        ...
        super_user_setup_available = await super_user_manager.can_create_super_user(
            session
        )
    ...
    try:
        ...
        for event in events:
            ...
            event.can_register = (
                event.status == EventStatus.PUBLISHED
                and (
                    not event.max_attendees
                    or event.attendee_count < event.max_attendees
                )
                and event.start_time > datetime.now(UTC)
            )
            event.is_full = (
                event.max_attendees and event.attendee_count >= event.max_attendees
            )
...
async def refresh_admin_stats(
    ...
    try:
        stats = await get_admin_dashboard_stats(session)
        return templates.TemplateResponse(
            "components/admin_stats.html", {"stats": stats}
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_admin_dashboard_stats(session: AsyncSession) -> dict[str, Any]:
    ...
    # Total attendees
    total_attendees_result = await session.execute(select(func.count(Attendee.id)))
...
async def get_user_dashboard_stats(
    ...
    total_matches_result = await session.execute(
        ...
        .join(
            Attendee,
            (Attendee.id == Match.attendee1_id) | (Attendee.id == Match.attendee2_id),
        )
        ...
        events.append(
            {
                ...
                "can_check_in": event.status == EventStatus.ACTIVE
                and not attendee.checked_in,
...
async def get_user_recent_matches(
