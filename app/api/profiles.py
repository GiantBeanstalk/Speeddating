...
from app.models import Attendee, Match, MatchResponse, User
...
async def validate_bio_content(
    ...
    response = BioValidationResponse(
        ...
        message=filter_result.violation_message
        if not filter_result.is_valid
        else "Bio content is valid",
    )
...
async def update_bio(
    ...
    if not attendee:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Attendee profile not found"
        )

    # Check authorization (only the attendee or event organizer can update)
    if (
        attendee.user_id != current_user.id
        and attendee.event.organizer_id != current_user.id
    ):
        raise HTTPException(
    ...
    return {"message": "Bio updated successfully", "bio": attendee.public_bio}
...
async def get_public_profile(
    ...
    attendee_result = await session.execute(
        select(Attendee)
        .options(selectinload(Attendee.event), selectinload(Attendee.user))
        .where(Attendee.id == attendee_id)
    )
    attendee = attendee_result.scalar_one_or_none()
    ...
    is_admin = current_user and (
        ...
        or attendee.event.organizer_id == current_user.id
    )
    ...
    if current_user:
        ...
        match_result = await session.execute(
            ...
            .join(
                Attendee,
                (Match.attendee1_id == Attendee.id)
                | (Match.attendee2_id == Attendee.id),
            )
            .where(
                Attendee.user_id == current_user.id,
                (
...
async def generate_profile_qr_token(
    ...
    qr_login.qr_code_url = (
        f"{base_url}/profiles/{attendee_id}?qr_token={qr_login.token}"
    )
    ...
    return ProfileQRResponse(
        profile_url=qr_login.qr_code_url, qr_token=qr_login.token, expires_in_hours=72
    )
...
async def access_profile_via_qr(
    ...
    validation_result = await qr_service.validate_qr_token(
        token=qr_token, event_id=attendee.event_id
    )
    ...
    if qr_login.attendee_id != attendee_id or qr_login.token_type != "profile_view":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    profile_data = attendee.get_public_profile_data(
        requester_is_admin=False, is_matched=False
    )
