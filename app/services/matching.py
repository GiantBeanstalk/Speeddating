...
from app.models import (
    Attendee, AttendeeCategory, Match, Round, Event,
    MatchResponse, RoundStatus
)
...
class MatchingService:
    ...
    def _group_by_category(self, attendees: List[Attendee]) -> Dict[AttendeeCategory, List[Attendee]]:
    ...
    def _calculate_capacity_limits(self, attendees_by_category: Dict[AttendeeCategory, List[Attendee]]) -> Dict[AttendeeCategory, int]:
        ...
        reciprocal_limits = {
            ...
            AttendeeCategory.TOP_MALE: min(
                counts[AttendeeCategory.TOP_FEMALE] + counts[AttendeeCategory.BOTTOM_FEMALE],
                counts[AttendeeCategory.TOP_MALE]
            ),
            
            # Top Female can match with Top Male and Bottom Male
            AttendeeCategory.TOP_FEMALE: min(
                counts[AttendeeCategory.TOP_MALE] + counts[AttendeeCategory.BOTTOM_MALE],
    ...
    async def get_round_statistics(self, round_id: uuid.UUID) -> Dict:
        ...
        for match in matches:
            pair_key = f"{match.attendee1.category.value} + {match.attendee2.category.value}"
            category_pairs[pair_key] = category_pairs.get(pair_key, 0) + 1
        
        response_rate = (completed_responses / total_matches * 100) if total_matches > 0 else 0
    ...
    async def validate_attendee_compatibility(
        ...
        attendees_result = await self.session.execute(
            select(Attendee)
            .where(Attendee.id.in_([attendee1_id, attendee2_id]))
        )
        attendees = list(attendees_result.scalars())
    ...
    async def get_event_matching_summary(self, event_id: uuid.UUID) -> Dict:
        ...
        return {
            ...
            "capacity_limits": {
                ...
                for category, limit in capacity_limits.items()
            },
            "max_possible_matches": max_possible_matches,
            "total_matches_created": total_matches,
            "mutual_matches": mutual_matches,
            "matching_efficiency": (total_matches / max_possible_matches * 100) if max_possible_matches > 0 else 0
        }
